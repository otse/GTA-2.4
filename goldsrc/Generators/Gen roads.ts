import Data2 from '../Objects/Data';
import Datas from '../Objects/Datas';

import Gen from './Gen';
import StagingArea from './Staging area';

export namespace GenRoads {

	export type Strings = 'badRoads' | 'greenRoads' | 'mixedRoads' | 'greyRoads';

	export function oneway(
		w: Gen.Truple, segs: number, sheet: Strings
	) {
		
	}

	export function onewayVert(
		w: Gen.Truple, segs: number, sheet: Strings
	) {

		let seg = 0;
		for (; seg < segs; seg++) {

			let road: Data2 = {
				type: 'Surface',
				sheet: sheet,
				square: 'single',
				x: w[0],
				y: seg + w[1],
				z: w[2],
				r: 3
			};

			if (!seg || seg == segs - 1) {
				road.square = 'singleOpen';

				if (!seg)
					road.r! += 1;

				else if (seg == segs - 1)
					road.r! -= 1;
			}

			Datas.replaceDeliver(road);

		}
	}

	export function onewayHorz(
		w: Gen.Truple, segs: number
	) {
		/*const lanes = 1;

		let seg = 0;
		for (; seg < segs; seg++) {

			let road: Data2 = {
				type: 'Surface',
				sheet: 'badRoads',
				square: 'single',
				x: seg + x,
				y: y,
				r: 0
			};

			Datas.Deliver(road);

			if (!seg || seg == segs - 1)
				road.square = 'singleOpen';
			
			if (!seg)
				road.r += 1;
			
			else if(seg == segs - 1)
				road.r -= 1; 

		}*/
	}

	export function twolane(
		axis: 0 | 1, w: [number, number, number], segs: number, sheet: Strings
	) {
		// Go 0 or 1

		console.log('twolane axis ', axis);

		let staging = new StagingArea;

		let datas = twolaneVert(w, segs, sheet);

		staging.addDatas(datas);

		staging.deliverAll();
	}

	export function twolaneVert(
		w: Gen.Truple, segs: number, sheet: Strings
	) {

		let datas: Data2[] = [];

		const lanes = 2;

		let seg = 0;
		for (; seg < segs; seg++) {

			let lane = 0;
			for (; lane < lanes; lane++) {

				let road: Data2 = {
					type: 'Surface',
					sheet: sheet,
					square: 'sideLine',
					x: lane + w[0],
					y: seg + w[1],
					r: !lane ? 3 : 1
				};

				if (!seg || seg == segs - 1) {
					road.square = 'convexLine';

					if (!seg && !lane ||
						seg == segs - 1 && lane)
						road.r! += 1;
				}

				else if (lane == lanes - 1 && seg == 1 ||
					!lane && seg == segs - 2) {
					road.square = 'sideStopLine';
				}

				datas.push(road);
				///Datas.replaceDeliver(road);
			}
		}

		return datas;

	}

	export function twolaneHorz(
		w: Gen.Truple, segs: number, sheet: Strings
		) {

		const lanes = 2;

		let seg = 0;
		for (; seg < segs; seg++) {

			let lane = 0;
			for (; lane < lanes; lane++) {

				let road: Data2 = {
					type: 'Surface',
					sheet: sheet,
					square: 'sideLine',
					x: seg + w[0],
					y: lane + w[1],
					r: !lane ? 2 : 0
				};

				if (!seg || seg == segs - 1) {
					road.square = 'convexLine';

					if (!seg && lane ||
						seg == segs - 1 && !lane)
						road.r! += 1;
				}

				else if (lane == lanes - 1 && seg == 1 ||
					!lane && seg == segs - 2) {
					road.square = 'sideStopLine'; // sideStopLine
					road.f = true;
				}

				///Datas.replaceDeliver(road);
			}
		}
	}

	// This is a same-way road
	export function highwayVert(
		w: Gen.Truple, segs: number, lanes: number
	) {

		let seg = 0;
		for (; seg < segs; seg++) {

			let lane = 0;
			for (; lane < lanes; lane++) {

				let road: Data2 = {
					type: 'Surface',
					sheet: 'badRoads',
					square: 'sideLine',
					x: lane + w[0],
					y: seg + w[1],
					r: !lane ? 3 : 1
				};

				if (lane > 0 && lane < lanes - 1)
					road.square = 'middleTracks';

				else if (!seg || seg == segs - 1) {
					road.square = 'convexLine';

					if (!seg && !lane ||
						seg == segs - 1 && lane)
						road.r! += 1;
				}

				/*else if (lane == lanes - 1 && seg == 1 ||
					!lane && seg == segs - 2) {
					road.square = 'sideStopLine';
				}*/

				///Datas.replaceDeliver(road);
			}
		}
	}

}

export default GenRoads;